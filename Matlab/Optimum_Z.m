...
EqUdif_rel = zeros(2,length(Zbio));
EqI_rel = zeros(1,length(Zbio));

DeltaRdac = 50;
Rdac = 5:DeltaRdac:1000;
Zbio_optimo = zeros(1,length(Rdac));
for j = 1:length(Rdac)
    ...
    for i = 1:length(Zbio)
        ...
        EqUdif_rel(1,i) = max(abs(EqUdif))/U_max;
        EqI_rel(1,i) = max(abs(EqI))/((U_max/(Ad*Zbio(i))*Rti));
        EqUdif_rel(2,i)= Zbio(i);
    end
    Sum_err_rel = abs(EqUdif_rel(1))+abs(EqI_rel);
    [EZbio_optimo,index] = min(Sum_err_rel);
    Zbio_optimo(1,j) = EqUdif_rel(2,index);
end

% -----------------------------FIG1------------------------------- %
figure(1)
plot(Rdac,Zbio_optimo,'o')
